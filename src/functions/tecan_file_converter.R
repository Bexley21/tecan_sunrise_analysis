...
tecan_file_converter <- function(file_source,duration,interval, save_location){
  data <- read_xlsx(file_source,sheet = 1)[,-1] #read in the .xlsx file and remove the column of row names
  rows <- c("A", "B","C","D","E","F","G","H")
  df <- matrix(seq(0,duration,by = interval)) #set a single column dataframe of the time values
  colnames(df) <- c("Time")
  ...
  for(i in 1:8) {
    ...
    df_time <- matrix(seq(0,duration,by = interval))
